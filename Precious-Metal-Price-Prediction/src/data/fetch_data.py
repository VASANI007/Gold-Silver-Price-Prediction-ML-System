...
def fetch_data(symbol, file_path):
    ...
    if os.path.exists(file_path):
        try:
            ...
            if old_data.empty:
                print(f"{file_path} is empty. Re-downloading...")
                raise Exception("Empty file")

            #  FIX 1: proper datetime
            old_data['Date'] = pd.to_datetime(old_data['Date'], errors='coerce')
            old_data = old_data.dropna(subset=['Date'])

            #  FIX 2: avoid duplicate date
            last_date = old_data['Date'].max() + pd.Timedelta(days=1)

            print(f"Updating {symbol} from {last_date.date()}...")

            new_data = yf.download(symbol, start=last_date)

            if new_data.empty:
                print("No new data available — keeping old data")
                return old_data
        ...
        #  FIX 4: sort
            ...
            print(f"Error → Re-downloading full data ({symbol})")
